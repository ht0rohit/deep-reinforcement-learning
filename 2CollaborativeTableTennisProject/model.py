...
class Actor(nn.Module):
	...
	def reset_parameters(self):
		"""Reset model parameters on each seperate training run"""
...
class Critic(nn.Module):
	...
	def __init__(self, state_size, action_size, seed, fcs1_units = 256, fc2_units = 128):
		...
		Params:
			...
			action_size (int) ---  dimension of each action
			seed (int)		  ---  random seed
			fcs1_units (int)  ---  no. of nodes in 1st hidden layer
			fc2_units (int)	  ---  no. of nodes in 2nd hidden layer
		"""
		...
		self.fcs1 = nn.Linear((state_size + action_size) * 2, fcs1_units) # *2:num_agents
